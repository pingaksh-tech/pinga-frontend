 ...
 >
   ...
   <div @click="clickGroup" class="group-header w-full">
     <span class="flex items-center w-full">
       <!-- Group Icon -->
       <feather-icon v-if="group.icon || this.groupIndex > Math.floor(this.groupIndex)" :icon="group.icon || 'CircleIcon'" :svgClasses="{ 'w-5 h-5 ': this.groupIndex % 1 != 0 }" />

       <!-- Group Name -->
       <span v-show="!verticalNavMenuItemsMin" class="truncate mr-3 select-none font-bold">{{ group.name }}</span>
   ...
   <ul ref="items" :style="styleItems" class="vs-sidebar-group-items">
     <li v-for="(groupItem, index) in group.submenu" :key="index">
       <!-- If item is group -->
       <v-nav-menu-group
         v-if="groupItem.submenu && checkAccess(groupItem)"
         :group="groupItem"
...
export default {
 ...
 methods: {
   checkAccess(groupItem) {
     if (!(this.checkRolesByIds(groupItem.roles || []) || this.checkPermissionSlug(groupItem.permissions || []))) {
       return false
     }
     return true
   },
