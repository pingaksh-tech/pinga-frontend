<template>
  <div class="vx-card p-6 upload-card" v-if="checkPermissionSlug(['PDF_Uploads_create'])">
    ...
    <div class="upload-area" @click="$refs.fileInput.click()" @dragover.prevent @drop="handleDrop">
      <input type="file" ref="fileInput" accept=".pdf" @change="handleFileChange" class="hidden" />
    ...
    <div class="button-group">
      <vs-button class="generate-button" @click="uploadPdf" :disabled="!isFormValid"> Generate Secure Link </vs-button>
      <vs-button color="warning" class="reset-button" @click="resetForm" :disabled="!hasData"> Reset </vs-button>
    </div>
    ...
    <div v-if="generatedLink" class="link-section">
      <div class="link-item">
        <div class="link-container">
          ...
          <vs-button @click="copyToClipboard(generatedLink.url)" class="copy-button"> Copy Link </vs-button>
        </div>
      </div>
      ...
      <div class="expiry-timer">
        ...
        <vs-checkbox v-model="linkExpired" disabled v-if="linkExpired"> Link has expired </vs-checkbox>
      </div>
    </div>
  </div>
</template>
