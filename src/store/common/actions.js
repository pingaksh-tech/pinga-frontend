...
export default {
  ...
  async getCollections({ commit }, params) {
    ...
    try {
      ...
      return {
        data: res.data.data.collections.map((c) => ({
          value: c._id,
          label: c.name
        })),
        message: res.data.message
  ...
  async getDiamondClarityDropdown({ commit }) {
    commit('SET_STATE', {
      action: 'listLoading',
      data: true
    })
    try {
      const res = await this.$http.get('diamond-clarity/', { params: { type: 'dropdown' } })
      console.log(res, 'res..........')
      ...
      // no need to return mapped array now, maybe return just message
